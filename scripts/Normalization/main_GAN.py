...
# sys.path.append("..")
#import python files
import model_GAN as modellib
import train_GAN as trainlib
...
class Normalize_Network(object):
	...
	def __init__(self, mode, config, model_dir = None, log_directory=None, weights = None):
		...
		"""
		assert mode in ['train', 'infer', 'generate', 'reconstruct', 'visualize', 'gradient', 'get_all_latents', 'train_GAN']
		if mode in ["infer", "reconstruct", "generate", "visualize", "gradient", "get_all_latents", "train_GAN"]:
		...
		if self.weights is not None:
			...
			if (mode != "train") and (mode != "train_GAN"):
	...
	def create_model(self):
		...
		self.optim = torch.optim.RMSprop(filter(lambda p: p.requires_grad, self.torch_model.parameters()), 
						lr=self.config.LEARNING_RATE, alpha=0.99, weight_decay=self.config.WEIGHT_DECAY, momentum=self.config.LEARNING_MOMENTUM)
		# self.optim = torch.optim.Adam(filter(lambda p: p.requires_grad, self.torch_model.parameters()),
        #                lr=self.config.LEARNING_RATE, weight_decay=self.config.WEIGHT_DECAY, betas=(self.config.BETA_1, self.config.BETA_2), amsgrad=True)
		...
		if self.mode in ["train"]:
			self.classifier_model = modellib.classifier(self.config)
			self.optim_C = torch.optim.RMSprop(filter(lambda p: p.requires_grad, self.classifier_model.parameters()), 
						lr=self.config.LEARNING_RATE, alpha=0.99, weight_decay=self.config.WEIGHT_DECAY, momentum=self.config.LEARNING_MOMENTUM)
			# self.optim_C = torch.optim.Adam(filter(lambda p: p.requires_grad, self.classifier_model.parameters()),
            #            lr=self.config.LEARNING_RATE, weight_decay=self.config.WEIGHT_DECAY, betas=(self.config.BETA_1, self.config.BETA_2), amsgrad=True)
			
			self.discriminator_model = modellib.discriminator_wasserstein(self.config)
			self.optim_D = torch.optim.RMSprop(filter(lambda p: p.requires_grad, self.discriminator_model.parameters()), 
						lr=self.config.LEARNING_RATE, alpha=0.99, weight_decay=self.config.WEIGHT_DECAY, momentum=self.config.LEARNING_MOMENTUM)
			# self.optim_D = torch.optim.Adam(filter(lambda p: p.requires_grad, self.discriminator_model.parameters()),
            #            lr=self.config.LEARNING_RATE, weight_decay=self.config.WEIGHT_DECAY, betas=(self.config.BETA_1, self.config.BETA_2), amsgrad=True)
	...
	def send_model_to_device(self):
		...
		if hasattr(self, 'discriminator_model'):
	...
	def load_dataset(self, source = None):
		...
		if self.mode in ['train','reconstruct','visualize', 'train_GAN']:
	...
	def load_weights(self):
		...
		if self.weights is not None:
			...
			""" There are other model parameters that ought to be saved, including classifier_state_dict, discriminator_state_dict"""
			#self.optim.load_state_dict(checkpoint['optimizer_state_dict']) #generally, we don't want to load the optimizer. Instead, we want it to start fresh.
	...
	def run_train(self):
		...
		assert hasattr(self, 'classifier_model'), "Classifier and optimizer has not yet been initialized. First run 'self.create_model'!"
		assert hasattr(self, 'discriminator_model'), "Discriminator and optimizer has not yet been initialized. First run 'self.create_model'!"
		...
		if self.weights is not None:
			...
			print("Initializing all model parameters to max range -{} to {}".format(0.08, 0.08))
			# Apply Xavier initialization to all layers of your model
			self.torch_model.apply(utilslib.xavier_init_range_all_layers)
			self.classifier_model.apply(utilslib.xavier_init_range_all_layers)
			self.discriminator_model.apply(utilslib.xavier_init_range_all_layers)

		device = self.send_model_to_device()

		self.optimizer_to(self.optim, device) #this is a function, defined below.
		self.optimizer_to(self.optim_C, device) 
		self.optimizer_to(self.optim_D, device) 
		...
		trainlib.train_v2(training_generator = self.train_gen, 
						validation_generator = self.val_gen,
						model = self.torch_model, 
						optimizer_G = self.optim,
						classifier = self.classifier_model, 
						optimizer_C = self.optim_C,
						discriminator = self.discriminator_model,
						optimizer_D = self.optim_D,
						log_path = self.logs,
						save_path = self.model_dir,
						device = device,
						config = self.config)

	def run_GAN_refine(self):
		import train_refine_GAN as trainlibGAN

		assert hasattr(self, 'train_gen'), "Data generators have not yet been initialized. First run 'self.load_dataset'!"
		assert hasattr(self, 'torch_model'), "Model and optimizer has not yet been initialized. First run 'self.create_model'!"
		assert self.mode == "train_GAN", "SAFETY WARNING: self.mode is set to '{}', but must be set to 'train' to run training.".format(self.mode)
		...
		#set the configuration so that the weight of the discriminator is just 1
		if self.config.train_opts.get('discriminator_weight') is not None:
			self.config.train_opts['discriminator_weight'] = 1. #set the weight to 1
		else:
			self.config.train_opts['discriminator_weight'] = 1. #set the weight to 1
		if self.config.train_opts.get('MSE_weight') is None:
			self.config.train_opts['MSE_weight'] = 1e-4

		self.config.STEPS_PER_EPOCH = 80
		self.config.VALIDATION_STEPS = 40
		self.config.train_opts['adversarial_switch_n'] = 2 #switch every other iteration.
		...
		trainlibGAN.train(training_generator = self.train_gen, 
		 			   validation_generator = self.val_gen,
					   model = self.torch_model, 
...
if __name__ == "__main__":
	...
	GN = Normalize_Network(mode='reconstruct', config=configurations.Config(), 
		    				 weights = "/home/groups/CEDAR/eddyc/projects/cyc_IF/DTRON2/scripts/Normalization/models/checkpoints/model_20231229T165017/Gland_net_Epoch_50.tar",
							 model_dir = "/home/groups/CEDAR/eddyc/projects/Nvidia-CEDAR/CEDAR-NVIDIA-Internal/Model/pseudotime_VAE/models/checkpoints", 
							 log_directory="/home/groups/CEDAR/eddyc/projects/Nvidia-CEDAR/CEDAR-NVIDIA-Internal/Model/pseudotime_VAE/models/logs")
	...
	# GN = Normalize_Network(mode='train_GAN', config=configurations.Config(), 
	# 						weights = "/home/groups/CEDAR/eddyc/projects/cyc_IF/DTRON2/scripts/Normalization/models/checkpoints/model_20231213T130613/Gland_net_Epoch_95.tar",
	# 						model_dir = "/home/groups/CEDAR/eddyc/projects/cyc_IF/DTRON2/scripts/Normalization/models/checkpoints", 
	# 						log_directory="/home/groups/CEDAR/eddyc/projects/cyc_IF/DTRON2/scripts/Normalization/models/logs")
	...
	if GN.mode == "train_GAN":
		print("running GAN refinement training...")
		GN.run_GAN_refine()
