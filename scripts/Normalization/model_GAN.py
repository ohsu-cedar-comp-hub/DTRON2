...
class Dec(nn.Module):
	...
	def __init__(self, config):
		...
		self.reshape_fc = nn.utils.parametrizations.spectral_norm(nn.Linear(style_channels, self.layer_dims[0] * self.spatial_dim * self.spatial_dim))
		self.reshapce_fc_act = nn.LeakyReLU(inplace=True)
		self.project_labels = nn.utils.parametrizations.spectral_norm(nn.Linear(sample_size, style_channels))
		self.project_labels_act = nn.LeakyReLU(inplace=True)
	...
	def forward(self, style, target):
		...
		target = self.project_labels_act(self.project_labels(target)) #now target has same dimensionality as style
		#let's try multiplying, or summing 
		# x = self.reshape_fc(torch.concat((style, target), axis=1))
		#import pdb;pdb.set_trace()
		x = self.reshapce_fc_act(self.reshape_fc(style + target))#torch.sum((style, target)))
...
class cycIF_VAE(nn.Module):
	...
	def forward(self, x, target, K=5, N=5): 
		...
		#probs = self.classifier(zs)#zs.mean(axis=0)) # I think we should do the classificaiton on the qz_x_params at this stage, rather than a randomly drawn sample.
		...
		return qz_x, px_z, zs #return posterior distribution, 
...
if __name__ == "__main__":
	...
	XR = cycIF_VAE(config)
	csfr = classifier(config)
	discrim = discriminator_wasserstein(config)
	...
	params = [zz.view(K, I.shape[0], *zz.shape[1:]) for zz in params]

	if XR.px_z.__name__ == "NegativeBinomial":
		px_z = XR.px_z(*reparameterize(*params)) #necessary for the negative binomial
	else:
		px_z = XR.px_z(*params)

	likelihood_batch_shape = px_z.batch_shape
	recon = utilslib.get_mean(px_z, K=5).view(likelihood_batch_shape[0] * likelihood_batch_shape[1], *(likelihood_batch_shape[2:]))
	#px_z.sample()#get_mean(px_z)
	logits = discrim(recon)
...
# ====================================================================================================
# ├─conv_tower: 1-1                                  [-1, 256, 8, 8]           --
# |    └─Sequential: 2-1                             [-1, 256, 8, 8]           --
# |    |    └─down_block: 3-1                        [-1, 16, 256, 256]        3,250
# |    |    └─AvgPool2d: 3-2                         [-1, 16, 128, 128]        --
# |    |    └─down_block: 3-3                        [-1, 32, 128, 128]        16,256
# |    |    └─AvgPool2d: 3-4                         [-1, 32, 64, 64]          --
# |    |    └─down_block: 3-5                        [-1, 64, 64, 64]          64,256
# |    |    └─AvgPool2d: 3-6                         [-1, 64, 32, 32]          --
# |    |    └─down_block: 3-7                        [-1, 128, 32, 32]         255,488
# |    |    └─AvgPool2d: 3-8                         [-1, 128, 16, 16]         --
# |    |    └─down_block: 3-9                        [-1, 256, 16, 16]         1,018,880
# |    |    └─AvgPool2d: 3-10                        [-1, 256, 8, 8]           -- #### INTERESTING THIS REDUCES DOWN TO 8
# ├─reduct_fc_AI: 1-2                                [-1, 1024]                --
# |    └─Flatten: 2-2                                [-1, 16384]               --
# |    └─ParametrizedLinear: 2-3                     [-1, 1024]                --
# ====================================================================================================
# Total params: 1,358,130
# Trainable params: 1,358,130
# Non-trainable params: 0
# Total mult-adds (M): 38.97
# ====================================================================================================
# Input size (MB): 1.50
...
# Layer (type:depth-idx)                                  Output Shape              Param #
# =========================================================================================================
# ├─ParametrizedLinear: 1-1                               [-1, 65536]               --
# |    └─ModuleDict: 2                                    []                        --
# |    |    └─ParametrizationList: 3-1                    [-1, 525]                 34,406,400
# ├─Sequential: 1-2                                       [-1, 16, 256, 256]        --
# |    └─up_block: 2-1                                    [-1, 128, 32, 32]         --
# |    |    └─resup_block: 3-2                            [-1, 128, 16, 16]         328,960
# |    |    └─Sequential: 3-3                             [-1, 128, 32, 32]         147,840
# |    └─up_block: 2-2                                    [-1, 64, 64, 64]          --
# |    |    └─resup_block: 3-4                            [-1, 64, 32, 32]          82,560
# |    |    └─Sequential: 3-5                             [-1, 64, 64, 64]          37,056
# |    └─up_block: 2-3                                    [-1, 32, 128, 128]        --
# |    |    └─resup_block: 3-6                            [-1, 32, 64, 64]          20,800
# |    |    └─Sequential: 3-7                             [-1, 32, 128, 128]        9,312
# |    └─up_block: 2-4                                    [-1, 16, 256, 256]        --
# |    |    └─resup_block: 3-8                            [-1, 16, 128, 128]        5,280
# |    |    └─Sequential: 3-9                             [-1, 16, 256, 256]        2,352
# ├─Sequential: 1-3                                       [-1, 3, 256, 256]         -- #### MU LIKELIHOOD
# |    └─BatchNorm2d: 2-5                                 [-1, 16, 256, 256]        32
# |    └─LeakyReLU: 2-6                                   [-1, 16, 256, 256]        --
# |    └─ParametrizedConv2d: 2-7                          [-1, 3, 256, 256]         --
# ├─Tanh: 1-4                                             [-1, 3, 256, 256]         -- #### ACTIVATION ON MU
# ├─Sequential: 1-5                                       [-1, 3, 256, 256]         -- #### SIGMOID LIKELIHOOD
# |    └─BatchNorm2d: 2-8                                 [-1, 16, 256, 256]        32
# |    └─LeakyReLU: 2-9                                   [-1, 16, 256, 256]        --
# |    └─ParametrizedConv2d: 2-10                         [-1, 3, 256, 256]         --
# =========================================================================================================
# Total params: 35,040,624
# Trainable params: 35,040,624
# Non-trainable params: 0
# Total mult-adds (M): 36.94
# =========================================================================================================
# Input size (MB): 0.00
# Forward/backward pass size (MB): 19.75
# Params size (MB): 133.67
# Estimated Total Size (MB): 153.42
# =========================================================================================================
...
# ===============================================================================================
# ├─Sequential: 1-1                             [-1, 13]                  --
# |    └─ParametrizedLinear: 2-1                [-1, 64]                  --
# |    └─LeakyReLU: 2-2                         [-1, 64]                  --
# |    └─Dropout: 2-3                           [-1, 64]                  --
# |    └─ParametrizedLinear: 2-4                [-1, 32]                  --
# |    └─LeakyReLU: 2-5                         [-1, 32]                  --
# |    └─Dropout: 2-6                           [-1, 32]                  --
# |    └─ParametrizedLinear: 2-7                [-1, 13]                  --
# ===============================================================================================
# Total params: 0
# Trainable params: 0
# Non-trainable params: 0
...
# |    |    └─AvgPool2d: 3-8                         [-1, 128, 16, 16]         --
# |    |    └─down_block: 3-9                        [-1, 256, 16, 16]         1,018,880
# |    |    └─AvgPool2d: 3-10                        [-1, 256, 8, 8]           --
# ├─reduct_fc_AI: 1-2                                [-1, 1]                   --
# |    └─Flatten: 2-2                                [-1, 16384]               --
# |    └─ParametrizedLinear: 2-3                     [-1, 1]                   --
# ====================================================================================================
# Total params: 1,358,130
# Trainable params: 1,358,130
# Non-trainable params: 0
# Total mult-adds (M): 5.45
# ====================================================================================================
# Input size (MB): 1.50
# Forward/backward pass size (MB): 0.00
# Params size (MB): 5.18
...
# Layer (type:depth-idx)                                  Output Shape              Param #
# =========================================================================================================
# ├─ParametrizedLinear: 1-1                               [-1, 512]                 --
# |    └─ModuleDict: 2                                    []                        --
# |    |    └─ParametrizationList: 3-1                    [-1, 13]                  6,656 ### Projecting the labels into the same dimensionality as the style
# ├─ParametrizedLinear: 1-2                               [-1, 65536]               --
# |    └─ModuleDict: 2                                    []                        --
# |    |    └─ParametrizationList: 3-2                    [-1, 512]                 33,554,432 # Doing the above operation reduces the parameters from 34,406,400 = 148,032 paramters less
# ├─Sequential: 1-3                                       [-1, 16, 256, 256]        --
# |    └─up_block: 2-1                                    [-1, 128, 32, 32]         --
# |    |    └─resup_block: 3-3                            [-1, 128, 16, 16]         328,960
# |    |    └─Sequential: 3-4                             [-1, 128, 32, 32]         147,840
# |    └─up_block: 2-2                                    [-1, 64, 64, 64]          --
# |    |    └─resup_block: 3-5                            [-1, 64, 32, 32]          82,560
# |    |    └─Sequential: 3-6                             [-1, 64, 64, 64]          37,056
# |    └─up_block: 2-3                                    [-1, 32, 128, 128]        --
# |    |    └─resup_block: 3-7                            [-1, 32, 64, 64]          20,800
# |    |    └─Sequential: 3-8                             [-1, 32, 128, 128]        9,312
# |    └─up_block: 2-4                                    [-1, 16, 256, 256]        --
# |    |    └─resup_block: 3-9                            [-1, 16, 128, 128]        5,280
# |    |    └─Sequential: 3-10                            [-1, 16, 256, 256]        2,352
# ├─Sequential: 1-4                                       [-1, 3, 256, 256]         --
# |    └─BatchNorm2d: 2-5                                 [-1, 16, 256, 256]        32
# |    └─LeakyReLU: 2-6                                   [-1, 16, 256, 256]        --
# |    └─ParametrizedConv2d: 2-7                          [-1, 3, 256, 256]         --
# ├─Tanh: 1-5                                             [-1, 3, 256, 256]         --
# ├─Sequential: 1-6                                       [-1, 3, 256, 256]         --
# |    └─BatchNorm2d: 2-8                                 [-1, 16, 256, 256]        32
# |    └─LeakyReLU: 2-9                                   [-1, 16, 256, 256]        --
# |    └─ParametrizedConv2d: 2-10                         [-1, 3, 256, 256]         --
# =========================================================================================================
# Total params: 34,195,312
# Trainable params: 34,195,312
# Non-trainable params: 0
